...
def save_cached_optimal_tour(map_path: str, req_path: str, tour: List[str], cost: float, 
                              num_permutations: int, computation_time: float) -> None:
...
def validate_path(path: List[str], valid_nodes: set) -> Tuple[bool, str]:
    ...
    if invalid_nodes:
        return False, f"Invalid nodes: {', '.join(invalid_nodes[:5])}" + (f" and {len(invalid_nodes) - 5} more" if len(invalid_nodes) > 5 else "")
    return True, ""
...
def input_manual_path(valid_nodes: set) -> List[str]:
    ...
    while True:
        ...
        if not path:
            print("Error: No valid nodes found. Please try again or press Enter to skip.")
            continue
...
def compare_paths(tsp_path: List[str], tsp_cost: float, 
                  manual_path: List[str], manual_cost: float,
                  manual_compact: Optional[List[str]] = None, manual_compact_cost: Optional[float] = None,
                  tsp_compact: Optional[List[str]] = None, tsp_compact_cost: Optional[float] = None,
                  optimal_path: Optional[List[str]] = None, optimal_cost: Optional[float] = None,
                  optimal_compact: Optional[List[str]] = None, optimal_compact_cost: Optional[float] = None):
    ...
    # Display TSP paths
    if tsp_compact and tsp_compact_cost is not None:
        print(f"\nTSP Heuristic (compact: {len(tsp_compact)} waypoints):")
        print(f"  {format_path(tsp_compact, max_display=40)}")
        print(f"  Direct cost: {tsp_compact_cost:.2f}")
        
        print(f"\nTSP Heuristic (expanded: {len(tsp_path)} nodes):")
        print(f"  {format_path(tsp_path)}")
        print(f"  Full cost: {tsp_cost:.2f}")
    else:
        print(f"\nTSP Heuristic (expanded: {len(tsp_path)} nodes):")
        print(f"  {format_path(tsp_path)}")
        print(f"  Cost: {tsp_cost:.2f}")
    
    # Display optimal solution if available
    if optimal_compact and optimal_compact_cost is not None:
        print(f"\nBrute-Force Optimal (compact: {len(optimal_compact)} waypoints):")
        print(f"  {format_path(optimal_compact, max_display=40)}")
        print(f"  Direct cost: {optimal_compact_cost:.2f}")
        
        if optimal_path and optimal_cost is not None:
            print(f"\nBrute-Force Optimal (expanded: {len(optimal_path)} nodes):")
            print(f"  {format_path(optimal_path)}")
            print(f"  Full cost: {optimal_cost:.2f}")
    
    # Display manual paths
    if manual_compact and manual_compact_cost is not None:
        print(f"\nManual Path (compact: {len(manual_compact)} waypoints):")
        print(f"  {format_path(manual_compact, max_display=40)}")
        print(f"  Direct cost: {manual_compact_cost:.2f}")
        
        print(f"\nManual Path (expanded: {len(manual_path)} nodes):")
        print(f"  {format_path(manual_path)}")
        print(f"  Full cost: {manual_cost:.2f}")
    else:
        print(f"\nManual Path ({len(manual_path)} nodes):")
        print(f"  {format_path(manual_path)}")
    ...
    if diff_heuristic == 0:
        ...
        print(f"  ★ BETTER: Your path is {abs(diff_heuristic):.2f} units shorter! ({abs(diff_heuristic/tsp_cost*100):.1f}% improvement)")
    else:
        print(f"  ✗ LONGER: Your path is {diff_heuristic:.2f} units longer ({diff_heuristic/tsp_cost*100:.1f}% more)")
    ...
    if optimal_cost is not None:
        ...
        if diff_optimal == 0:
            ...
            print(f"  ★★ BETTER: Your path is {abs(diff_optimal):.2f} units shorter than optimal! (check for errors)")
        else:
            print(f"  Gap to optimal: {diff_optimal:.2f} units ({diff_optimal/optimal_cost*100:.1f}% suboptimal)")
    ...
    if tsp_path == manual_path:
        ...
        common_count = sum(1 for i in range(min(len(tsp_path), len(manual_path))) 
...
def main():
    parser = argparse.ArgumentParser(description='Compare manual path with TSP solution')
    parser.add_argument('--map', type=str, default=None, help='Map XML file to use')
    parser.add_argument('--req', '--delivery', type=str, default=None, dest='req', 
                        help='Delivery requests XML file (contains <livraison> elements)')
    parser.add_argument('--nodes', type=int, default=0, help='Limit number of nodes (0 = use all from requests)')
    ...
    if args.map:
        ...
        tsp._build_networkx_map_graph = lambda xml_file_path=None, p=args.map: orig_builder(p)
    ...
    if args.req:
        ...
        try:
            ...
            for i, d in enumerate(deliveries[:5], 1):
                ...
                print(f"  {i}. Pickup: {pickup} -> Delivery: {delivery} "
            ...
            nodes_list = list(all_nodes)[:args.nodes] if args.nodes > 0 else list(all_nodes)
    else:
    ...
    print(f"  {', '.join(nodes_list[:20])}" + (f" ... and {len(nodes_list) - 20} more" if len(nodes_list) > 20 else ""))
    ...
    print(f"  Expanded path: {len(tsp_full_route)} nodes, cost: {tsp_expanded_cost:.2f}")
    ...
    if args.optimal:
        if not BRUTE_FORCE_AVAILABLE:
            ...
            if cached_optimal:
                ...
                print(f"{'='*70}")
                print(f"✓ Found cached optimal tour!")
                print(f"  Computed: {cached_optimal.get('computed_at', 'unknown')}")
                print(f"  Computation time: {cached_optimal.get('computation_time_s', 0):.2f}s")
                print(f"  Permutations checked: {cached_optimal.get('num_permutations_checked', 0):,}")
                
                optimal_tour = cached_optimal['tour']
                optimal_compact_cost = cached_optimal['cost']
                
                print(f"  Compact tour: {len(optimal_tour)} nodes, cost: {optimal_compact_cost:.2f}")
                print(f"  Tour order: {' -> '.join(optimal_tour)}")
                
                # Expand the cached tour
                print("  Expanding cached tour to full path...")
                try:
                    optimal_full_route, optimal_expanded_cost = tsp.expand_tour_with_paths(optimal_tour, sp_graph)
                    print(f"  Expanded path: {len(optimal_full_route)} nodes, cost: {optimal_expanded_cost:.2f}")
                    
                    # Compare with heuristic
                    improvement = compact_cost - optimal_compact_cost
                    if improvement > 0.01:
                        print(f"  ⚠️  Heuristic was {improvement:.2f} units longer ({improvement/optimal_compact_cost*100:.1f}% suboptimal)")
                    else:
                        print(f"  ✓ Heuristic found the optimal solution!")
                except Exception as e:
                ...
                print(f"{'='*70}")
                print(f"This will check all valid permutations for {len(nodes_list)} nodes...")
                ...
                try:
                    ...
                    for candidate_tour in generate_all_valid_tours(tour_pairs, start_node=None):
                        count += 1
                    ...
                    if best_tour:
                        ...
                        if args.map and args.req:
                            save_cached_optimal_tour(args.map, args.req, optimal_tour, 
                                                     optimal_compact_cost, count, elapsed)
    ...
    manual_path_expanded, manual_expanded_cost = expand_manual_path(manual_path_compact, G_map)
    ...
    compare_paths(tsp_full_route, tsp_expanded_cost, 
                  manual_path_expanded, manual_expanded_cost,
                  manual_path_compact, manual_compact_cost,
                  tour, compact_cost,
                  optimal_full_route, optimal_expanded_cost,
                  optimal_tour, optimal_compact_cost)
