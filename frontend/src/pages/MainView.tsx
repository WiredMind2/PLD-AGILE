        ...
        setDeliveryPoints((prev) => {
          ...
          deliveries.forEach((d: any) => {
            const pickup = map.intersections.find(
              (i) => String(i.id) === String(d.pickup_addr?.id ?? d.pickup_addr)
            );
            const drop = map.intersections.find(
              (i) =>
                String(i.id) === String(d.delivery_addr?.id ?? d.delivery_addr)
            );
            if (pickup) {
              base.push({
                id: `pickup-${d.id}`,
                        ...
                        try {
                          if (map && Array.isArray(map.deliveries)) {
                            ...
                            if (
                              match &&
                              match.warehouse &&
                              typeof match.warehouse.latitude === "number" &&
                              typeof match.warehouse.longitude === "number"
                            ) {
                              return [
                                match.warehouse.latitude,
                                match.warehouse.longitude,
                              ] as [number, number];
                            }
                          }
                        } catch (e) {
              ...
              <div className="space-y-3">
                ...
                <div className="max-h-56 overflow-auto rounded-md border border-emerald-200 dark:border-emerald-800 divide-y divide-emerald-100 dark:divide-emerald-900">
                  {(deliveries || []).map((d: any) => {
                    const pickupId =
                      typeof d.pickup_addr === "string"
                        ? d.pickup_addr
                        : d.pickup_addr?.id;
                    const deliveryId =
                      typeof d.delivery_addr === "string"
                        ? d.delivery_addr
                        : d.delivery_addr?.id;
                    return (
                      <div
